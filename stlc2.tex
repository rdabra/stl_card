...
\usepackage{listings}
\lstdefinestyle{mystyle}{language=C++,
...
\begin{document}
\noindent
{\large{\textbf{\emph{C++ Standard Library Cheat Sheet}}, {\small version 1.5}}	
}

\noindent
{\small  Roberto D. Algarte (2026)}
...
\begin{itemize}[leftmargin=*,topsep=0.25pt]
  ...
  \item \emph{\textbf{std::erase / std::erase\_if}} (C++20): convenient erase for \textbf{vector} (\emph{no erase-remove boilerplate});
\end{itemize}
...
\begin{itemize}[leftmargin=*,topsep=0.25pt]
  ...
  \item \emph{\textbf{std::erase / std::erase\_if}} (C++20): convenient erase for \textbf{deque};
\end{itemize}
...
\begin{itemize}[leftmargin=*,topsep=0.25pt]
  ...
  \item unordered\_map::\emph{\textbf{contains}} (C++20): membership test (no iterator);
...
\begin{itemize}[leftmargin=*,topsep=0.25pt]
  \setlength\itemsep{-1.8pt}
\item  {\textbf{expected}} @ $<$expected$>$ (C++23): declares a variable that has a pair of expected and unexpected values. It is useful for certain functions returning values;
...
\subsubsection*{\textsc{numeric\_limits} @ $<$limits$>$} 
\begin{itemize}[leftmargin=*,topsep=0.25pt]
  \setlength\itemsep{-1.8pt}
  \item numeric\_limits<T>::\emph{\textbf{min}}: smallest positive finite value for floating types; smallest finite value for integer types;
  \item numeric\_limits<T>::\emph{\textbf{lowest}} (C++11): lowest finite value representable by \textbf{T};
  \item numeric\_limits<T>::\emph{\textbf{max}}: largest finite value representable by \textbf{T};
  \item numeric\_limits<T>::\emph{\textbf{epsilon}}: distance between 1 and the next representable floating-point value;
  \item numeric\_limits<T>::\emph{\textbf{infinity}}: positive infinity value if supported;
  \item numeric\_limits<T>::\emph{\textbf{quiet\_NaN}}: quiet NaN value if supported;
  \item numeric\_limits<T>::\emph{\textbf{signaling\_NaN}}: signaling NaN value if supported;
  \item numeric\_limits<T>::\emph{\textbf{denorm\_min}}: smallest positive subnormal value for floating-point types;
  \item numeric\_limits<T>::\textbf{is\_signed}: \textbf{true} if \textbf{T} is signed;
  \item numeric\_limits<T>::\textbf{is\_integer}: \textbf{true} if \textbf{T} is an integer type;
  \item numeric\_limits<T>::\textbf{is\_exact}: \textbf{true} if \textbf{T} represents values exactly;
  \item numeric\_limits<T>::\textbf{digits}: number of radix digits that can be represented without change;
  \item numeric\_limits<T>::\textbf{digits10}: number of base-10 digits that can be represented without change;
  \item numeric\_limits<T>::\textbf{max\_digits10} (C++11): number of base-10 digits required to uniquely represent \textbf{T};
  \item numeric\_limits<T>::\textbf{has\_infinity}: \textbf{true} if infinity is supported;
  \item numeric\_limits<T>::\textbf{has\_quiet\_NaN}: \textbf{true} if quiet NaN is supported;
\end{itemize}

\subsubsection*{\textsc{print functions} @ $<$print$>$} 
\begin{itemize}[leftmargin=*,topsep=0.25pt]
  \setlength\itemsep{-1.8pt}
  \item \emph{\textbf{print}} (C++23): writes formatted text to \textbf{stdout};
  \item \emph{\textbf{print}} \emph{(FILE*, fmt, args...)} (C++23): writes formatted text to a specified C stream such as \textbf{stderr};
  \item \emph{\textbf{println}} (C++23): same as \emph{\textbf{print}}, but appends a newline at the end;
  \item \emph{\textbf{println}} \emph{(FILE*, fmt, args...)} (C++23): same as \emph{\textbf{print}} to a specified stream, but appends a newline;
  \item \textbf{\{\}}: default placeholder; arguments fill placeholders from left to right;
  \item \textbf{\{\{\} and \}\}}: escaped braces for printing literal \textbf{\{} and \textbf{\}};
  \item \textbf{\{:<n\}} / \textbf{\{:>n\}} / \textbf{\{:\^{}n\}}: left/right/center alignment in a field of width \textbf{n};
  \item \textbf{\{:0n\}}: zero-padded integer field of width \textbf{n};
  \item \textbf{\{:.pf\}}: floating-point fixed notation with \textbf{p} digits after decimal point;
  \item \textbf{\{:.pe\}} / \textbf{\{:.pE\}}: floating-point scientific notation with precision \textbf{p};
  \item \textbf{\{:.pg\}} / \textbf{\{:.pG\}}: floating-point general notation with precision \textbf{p};
  \item \textbf{\{:b\}} / \textbf{\{:o\}} / \textbf{\{:x\}} / \textbf{\{:X\}}: binary/octal/hexadecimal integer formatting;
  \item \textbf{\{:*\^{}n\}}: example of custom fill character (\textbf{*}) combined with alignment and width;
\end{itemize}
