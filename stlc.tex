...
\begin{itemize}[leftmargin=*,topsep=0.25pt]
  \setlength\itemsep{-1.8pt}
	...
	\item container::\emph{\textbf{erase}} @ $<$container$>$: deletes elements in a specified range;
...
\subsubsection*{\textsc{Practical Member Functions (day-to-day)}} 
\noindent These are the ones you end up reaching for constantly.

\begin{itemize}[leftmargin=*,topsep=0.25pt]
  \setlength\itemsep{-1.8pt}
  \item container::\emph{\textbf{size}} @ $<$container$>$: number of elements currently stored;
  \item container::\emph{\textbf{empty}} @ $<$container$>$: checks if \emph{size == 0};
  \item container::\emph{\textbf{begin/end}} @ $<$container$>$: iterators to the first element / one-past-the-last element;
  \item container::\emph{\textbf{cbegin/cend}} @ $<$container$>$: const iterators (read-only);
  \item container::\emph{\textbf{front/back}} @ $<$container$>$: references to first/last element (\textbf{precondition:} not empty);
  \item container::\emph{\textbf{swap}} @ $<$container$>$: swaps contents in $O(1)$ (typically by swapping pointers/metadata);
\end{itemize}
...
\begin{itemize}[leftmargin=*,topsep=0.25pt]
  ...
  \item unordered\_map::\emph{\textbf{try\_emplace}} (C++17): inserts only if missing; constructs value only on insertion;
  \item unordered\_map::\emph{\textbf{insert\_or\_assign}} (C++17): insert if missing, otherwise overwrite value;
  ...
  \item unordered\_map::\emph{\textbf{extract/merge}} (C++17): node handles (move elements between maps without reallocation);
\end{itemize}
...
\item Code example:
{\fontfamily{zi4}\selectfont \lstset{style=mystyle}
...
\begin{itemize}[leftmargin=*,topsep=0.25pt]
  \setlength\itemsep{-1.8pt}
	...
	\item  \emph{\textbf{ranges::unique}} @ $<$algorithm$>$: removes duplicated values and returns a pair of [begin,end] iterators of the excess tail;
...
\begin{itemize}[leftmargin=*,topsep=0.25pt]
  \setlength\itemsep{-1.8pt}
	...
	\item  \emph{\textbf{ranges::max\_element}} @ $<$algorithm$>$: returns the address of the biggest value in a container;  
	\item  \emph{\textbf{min}} @ $<$algorithm$>$: returns the smallest value between two values or in a initializer list;  
	\item  \emph{\textbf{ranges::min\_element}} @ $<$algorithm$>$: returns the address of the smallest value in a container;  
	\item  \emph{\textbf{ranges::minmax\_element}} @ $<$algorithm$>$: returns a pair of address of the smallest and biggest values in a container;  	
...
\begin{itemize}[leftmargin=*,topsep=0.25pt]
  \setlength\itemsep{-1.8pt}
	...
	\item  {\textbf{weak\_ptr}} @ $<$memory$>$: smart pointer in which copy is enabled. The number of copies is not registered. Dangling pointers can be checked.
\end{itemize}
...
\subsubsection*{\textsc{String functions}}
\begin{itemize}[leftmargin=*,topsep=0.25pt]
\setlength\itemsep{-1.8pt}
